...
export const addToCart = (id: string, qty: number) => async (dispatch: $TSFixMe, getState: $TSFixMe) => {
  try {
...
export const incrementProductQuantity = (id: string, currentqty: number, total: number) => (dispatch: $TSFixMe, getState: $TSFixMe) => {
...
export const decrementProductQuantity = (id: string, currentqty: number) => (dispatch: $TSFixMe, getState: $TSFixMe) => {
  if (currentqty <= 1) {
    dispatch({
      type: CART_ITEM_QUANTITY_FAILURE,
